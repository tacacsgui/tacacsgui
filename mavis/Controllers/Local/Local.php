...
class Local extends Controller
{
  ...
  private function dPrefix()
  {
    ...
    $this->user = $this->db->table('tac_users')->select('id','login', 'login_flag', 'login_change')->whereRaw("BINARY `username`='{$this->mavis->getUsername()}'")->where([ ['login_flag', 3], ['mavis_otp_enabled', 0], ['mavis_sms_enabled', 0] ]);
