...
private $listOfTacacsReportsTables = '--tables tac_log_accounting tac_log_authorization tac_log_authentication';
	...
	###################	MAKE Backup########START##
	public function makeBackup($attrArray)
	{
		$backupPart="all";
		if (!empty($attrArray['make'])) $backupPart=$attrArray['make'];
	...
	public function postBackupDatatables($req,$res)
	{
		...
		{	
			...
			$tempArray['buttons']='<button class="btn btn-warning btn-xs btn-flat" onclick="restoreBackup(\''.$data['result4'][$i].'\',\''.$deviceGroup['name'].'\')">Restore</button> <button class="btn btn-danger btn-xs btn-flat" onclick="deleteBackup(\''.$data['result4'][$i].'\',\''.$deviceGroup['name'].'\')">Del</button>';
	...
	public function postBackupDelete($req,$res)
	{
		...
		$data['deleteBackup'] = trim(shell_exec(TAC_ROOT_PATH . '/backup.sh delete '. $req->getParam('name'))); 
	...
	public function postBackupRestore($req,$res)
	{
		...
		$data['restore'] = trim(shell_exec(TAC_ROOT_PATH . '/backup.sh restore '. DB_USER . ' ' . DB_PASSWORD . ' '. DB_NAME. ' '. $req->getParam('name'))); 
