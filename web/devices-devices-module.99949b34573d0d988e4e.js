...
var EditComponent = /** @class */ (function () {
    ...
    EditComponent.prototype.fillForm = function (id) {
        ...
        this.dev_service.get(id).subscribe(function (data) {
            ...
            rxjs_1.forkJoin(_this.pre.get('/tacacs/device/group/list/', data.group.join(',')), _this.pre.get('/tacacs/acl/list/', data.acl.join(',')), _this.pre.get('/tacacs/user/group/list/', data.user_group.join(','))).pipe(operators_1.map(function (_a) {
                var group = _a[0], acl = _a[1], user_group = _a[2];
                console.log(group, acl, user_group);
                var returnData = {};
                if (group)
    ...
    function TacDeviceFormComponent(toastr) {
        ...
        this.list = {
            ...
            address: {
                ...
                pop: {
                    content: function (item, i) {
                        return '<table>' +
                            '<tr><td>Name:</td><td><b>' + item.text + '</b></td></tr>' +
                            '<tr><td>Type:</td><td><b>' + ((!!item.type) ? 'IPv6' : 'IPv4') + '</b></td></tr>' +
                            '<tr><td>Address:</td><td><b>' + item.address + '</b></td></tr>' +
                            '</table>';
                    },
                }
            },
...
//# sourceMappingURL=devices-devices-module.99949b34573d0d988e4e.js.map