...
var AddComponent = /** @class */ (function () {
    ...
    AddComponent.prototype.add = function () {
        ...
        console.log(data);
        data.service = (data.service[0] && data.service[0].id) ? data.service.map(function (x) { return x.id; }) : null;
...
var EditComponent = /** @class */ (function () {
    ...
    EditComponent.prototype.fillForm = function (id) {
        ...
        this.usr_service.get(id).subscribe(function (data) {
            ...
            //console.log(data)
            rxjs_1.forkJoin(_this.pre.get('/tacacs/user/group/list/', data.group.join(',') || 0, { type: 'user', id: data.id }), _this.pre.get('/tacacs/acl/list/', data.acl.join(',')), _this.pre.get('/tacacs/device/list/', data.device_list.join(',') || 0), _this.pre.get('/tacacs/device/group/list/', data.device_group_list.join(',') || 0)).pipe(operators_1.map(function (_a) {
                var group = _a[0], acl = _a[1], device_list = _a[2], device_group_list = _a[3];
    ...
    EditComponent.prototype.save = function () {
        ...
        if (data.enable == old_data.enable && data.enable_flag == old_data.enable_flag)
            delete data.enable;
        if (data.pap == old_data.pap && data.pap_flag == old_data.pap_flag)
            delete data.pap;
        if (data.login == old_data.login && data.login_flag == old_data.login_flag)
            delete data.login;
        console.log(data);
        data.service = (data.service[0] && data.service[0].id) ? data.service.map(function (x) { return x.id; }) : null;
        data.group = (data.group[0] && data.group[0].id) ? data.group.map(function (x) { return x.id; }) : null;
...
//# sourceMappingURL=users-users-module.5884b91ec20c70a80fc2.js.map